...
public class GameServiceImpl implements GameService {
	...
	public void updateScore(Game game) {
		...
		if(question.isResponseValid()) {
			game.incrementScore();
			Utils.printColor(" Bravo ! ", Color.GREEN);
		} else {
			Utils.printColor(" Ce n’est pas la bonne réponse. ", Color.RED);
		}
	...
	public void printScoreFinal(Game game) {
		...
		game.getQuestions().stream().map(Question::getVerbToString).forEach(System.out::println);
	}
	
	public float meanTimeOfResponse(Game game) {
		return (float) (game.getQuestions().stream().mapToLong(Question::getTimeResponse).sum())/game.getNumberOfQuestion()/1000;
	}
	
	public void initRandomVerbs(Game game) {
		int numberOfQuestion = game.getNumberOfQuestion();
		List<Verb> verbs = verbService.getRandomVerbs(numberOfQuestion);
